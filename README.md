...
On a desktop with a large monitor, the layout appears centered and well-proportioned. The content fits comfortably within the 960px container and the two-column structure is clearly visible.
...
On a smartphone, the problem becomes more severe. The 960px container is significantly wider than the screen, so the user sees only the left portion of the layout and must scroll horizontally to access the rest. This is a very poor user experience.
...
Another issue was keeping the screen width under control. I initially set the container to 100% width by mistake and had to reduce it to a fixed pixel value to demonstrate the fixed layout approach correctly.
...
## 2. Fluid Layout (fluid.html)

In the fluid layout, all dimensions are defined as percentages relative to the viewport width. The main container is set to 90% of the viewport width, the navigation links are 22% each, and the two columns are 65% and 30% of the container.
...
## 3. Adaptive Layout (adaptive.html)
...
**New knowledge:** During this task I became familiar with two new CSS techniques. The first is the @media rule, which allows you to apply different CSS styles depending on the viewport width. Using max-width inside a media query means the styles inside it will apply only when the viewport is at or below that specified width. This is the key tool that makes both adaptive and responsive layouts possible. The second technique is the clearfix method. When using float: left and float: right to create columns, the parent container collapses and does not wrap around the floated children, which causes the footer to overlap with the content. The clearfix solves this by adding a ::after pseudo-element with clear: both and display: table to the parent wrapper, which forces the container to correctly calculate its full height and push the footer below the columns.
...
## 4. Responsive Layout (responsive.html)
...
On a large desktop, the content fills a comfortable portion of the screen up to 1100px. On a medium laptop, it scales down proportionally. On a tablet near the breakpoint, the transition is smooth and the layout restructures clearly. On a smartphone, the single-column layout is easy to scroll and read.
